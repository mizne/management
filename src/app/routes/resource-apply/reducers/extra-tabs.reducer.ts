...
export function reducer(state: State = initialState, action: Actions): State {
    switch (action.type) {
        ...
        case fromExtraTabs.CANCEL_EDIT_REQUIREMENT_APPLY:
            return ExtraTabsHelper.generateForCancelEdit(state, action.tabIndex)
        case fromExtraTabs.ENSURE_EDIT_REQUIREMENT_APPLY:
            return ExtraTabsHelper.generateForEnsureEdit(state, action.tabIndex)
        case fromExtraTabs.ENSURE_EDIT_REQUIREMENT_APPLY_SUCCESS:
            return ExtraTabsHelper.generateForEnsureEditSuccess(
                state,
                action.tabIndex
            )
        case fromExtraTabs.ENSURE_EDIT_REQUIREMENT_APPLY_FAILURE:
